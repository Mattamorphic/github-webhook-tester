...
class HookManager {
  ...
  /**
   * ASYNC Setup the hook manager, loading in the current hooks
   */
  async setup() {
  ...
  /**
   * ASYNC Check if a hook exists, and remove it
   *
    ...
    endpoints.forEach(async (endpoint, i) => {
      Object.keys(hookUrlToId).forEach(async (hookEndpoint) => {
        ...
        if (hookEndpoint.includes(`ngrok.io/${endpoint}`)) {
          logger.debug(`${i+1} Removing ${hookEndpoint}`);
          await this.github.genDeleteHook(hookUrlToId[hookEndpoint]);
          logger.debug(`${i+1} Removed ${hookEndpoint}`);
        }
      });
    });
  }
  ...
  /**
   * ASYNC Delete all ngrok hooks
   *
   */
  async genDeleteAllNgrokHooks() {
    const hookUrlToId = Object.assign(
        ...this.hooks.map((hook) => ({[hook.config.url]: hook.id})),
    );
    Object.keys(hookUrlToId).forEach(async (hookEndpoint, i) => {
      if (!hookEndpoint.includes('ngrok.io')) {
        return;
      }
      logger.debug(`${i+1} Removing ${hookEndpoint}`);
      await this.github.genDeleteHook(hookUrlToId[hookEndpoint]);
  ...
  /**
   * ASYNC Create hooks from the given spec
   *
  ...
  async genCreateWebhooks(url, spec) {
    Object.keys(spec).forEach(async (endpoint, i) => {
      ...
      logger.debug(`${i+1} Creating ${config.url}`);
      await this.github.genCreateHook(config);
      logger.debug(`${i+1} Created ${config.url}`);
    });
  }
}
